...
# -*- coding: utf-8 -*-
"""
Created on Mon Jul 18 14:11:22 2022
...
def main(inputs, mode, method, args):
    ...
    indiv_output = {}
    trans_dic = joblib.load("/homes/GPU2/shaohao/turn_taking/turn_changing/final/transition/output_transition/output_trans_cv3.pkl")
    ...
    for i, dia in enumerate(dialogs):
        ...
        for t in range(len(dialogs[dia])):
            # pdb.set_trace()
            trans_idx = dia+'_'+str(t).zfill(3)
            if t == 0:
                DED.transition_bias = 0.95
            else:
                if trans_dic[trans_idx] >= 0.05:
