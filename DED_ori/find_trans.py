...
# -*- coding: utf-8 -*-
"""
Created on Tue Jul 12 14:18:32 2022
...
def run(trans):
    ...
    with open('./trans.csv', 'a', newline='') as x:
        writer = csv.writer(x)
        writer.writerow(['Transit', 'pred', args.transition_bias, acc, f1, precision, uar])
    ...
    with open('./trans.csv', 'a', newline='') as x:
        writer = csv.writer(x)
        writer.writerow(['Transit', 'origin', args.transition_bias, acc, f1, precision, uar])
    ...
    with open('./trans.csv', 'a', newline='') as x:
        writer = csv.writer(x)
        writer.writerow(['same', 'pred', args.transition_bias, acc, f1, precision, uar])
    
    
    acc, uar, precision, f1, conf = utils.evaluate(same_gt_lst, same_org_lst)
    print('Same origin performance: uar: {:.4f}, acc: {:.4f}, f1: {:.4f}, precision: {:.4f}\n'.format(uar, acc, f1, precision))
    
    
    with open('./trans.csv', 'a', newline='') as x:
       writer = csv.writer(x)
       writer.writerow(['same', 'origin', args.transition_bias, acc, f1, precision, uar])


for trans in range(5,100,5):
    run(trans/100.0)    
