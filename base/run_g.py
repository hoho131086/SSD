...
# -*- coding: utf-8 -*-
"""
Created on Thu Jun 23 23:05:56 2022
...
import tqdm
from model_g import ATT_TALK, ATT_OTHER_GAZE, ATT_ME_GAZE, ATT_Combine, ATT_Combine_Me, ATT_ME_OTHER, ATT_Talk_Me
...
parser.add_argument('--date', type=str,  default="0705",help='model select')
...
for fold, (train_index, valid_index) in enumerate(skf.split(TRAIN_RECORDING)):
    ...
    if args.model == 'ATT_TALK': #(fea_dim, hidden_dim, tfm_head, out_dim, dropout, func)
        model = ATT_TALK(fea_dim = 1, hidden_dim = 4, layer_num = 1, tfm_head = 2, out_dim = 1).to(device)
    elif args.model == 'ATT_OTHER_GAZE': #(self, fea_dim, hidden_dim, layer_num, tfm_head, out_dim, dropout, func):
        model = ATT_OTHER_GAZE(fea_dim=1, hidden_dim=16,layer_num = 1, tfm_head = 8, out_dim = CLASS_NUM, dropout=args.dropout, func=args.func).to(device)
    elif args.model == 'ATT_ME_GAZE': #(self, fea_dim, hidden_dim, layer_num, tfm_head, out_dim, dropout, func):
        model = ATT_ME_GAZE(fea_dim=4, hidden_dim=4,layer_num = 1, tfm_head = 2, out_dim = CLASS_NUM).to(device)
    elif args.model == 'ATT_Combine': #(self, talk_fea, talk_hidden, gaze_fea, gaze_hidden, layer_num, tfm_head, out_dim):
        model = ATT_Combine(talk_fea=1, talk_hidden=4, gaze_fea = 1, gaze_hidden = 16,
                            layer_num = 1, tfm_head_talk = 2, tfm_head_gaze = 8, out_dim = CLASS_NUM).to(device)
    elif args.model == 'ATT_Combine_Me': 
    ...
    elif args.model == 'ATT_ME_OTHER': # me_gaze_fea, me_gaze_hidden, gaze_fea, gaze_hidden, layer_num, tfm_head_gaze, head_me_gaze, out_dim
        model = ATT_ME_OTHER(me_gaze_fea = 4, me_gaze_hidden = 4, gaze_fea = 1, gaze_hidden = 16,
                             layer_num = 1, tfm_head_gaze = 8, head_me_gaze = 2, out_dim = CLASS_NUM).to(device)

    elif args.model == 'ATT_Talk_Me': # talk_fea, talk_hidden, me_gaze_fea, me_gaze_hidden, layer_num, tfm_head_talk, head_me_gaze, out_dim
        model = ATT_Talk_Me(talk_fea = 1, talk_hidden = 4, me_gaze_fea = 4, me_gaze_hidden = 4,
                             layer_num = 1, tfm_head_talk = 2, head_me_gaze = 2, out_dim = CLASS_NUM).to(device)
