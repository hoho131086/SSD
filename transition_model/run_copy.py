...
# -*- coding: utf-8 -*-
"""
Created on Thu Sep 22 15:48:47 2022
...
import tqdm
from model import ATT_CHAR_OTHER, ATT_CHAR_OTHER_V2, ATT_NEW, ATT_NEW_2
...
parser.add_argument('--model', type=str,  default="new_model_v4",help='model select')
...
for fold, (train_index, valid_index) in enumerate(skf.split(TRAIN_RECORDING)):
    ...
    if args.model == 'new_model_v1':
        ...
        model = ATT_NEW(talk_fea=1, talk_hidden=8, me_gaze_fea = 4, me_gaze_hidden = 8, gaze_fea = 1, gaze_hidden = 8, char_hidden=8,
                               layer_num = 1, tfm_head_talk = 2, tfm_head_gaze = 8, head_me_gaze = 2, out_dim = CLASS_NUM).to(device)
    elif args.model == 'new_model_v4':
        model = ATT_NEW_2(talk_fea=1, talk_hidden=8, me_gaze_fea = 4, me_gaze_hidden = 8, gaze_fea = 1, gaze_hidden = 8, char_hidden=8,
    ...
    plt.ylabel('loss')
    plt.legend()
    plt.savefig(os.path.join(IMG_SAVE,'cv_{}.png'.format(fold)))
    # plt.show()
    ...
    with open(RESULT_SAVE, 'a', newline='') as x:
        writer = csv.writer(x)
        writer.writerow([args.epoch, args.label_type, args.char, args.model, LR, ACC, f1score, precision, UAR])
  # writer.writerow(['epoch', 'label_mode', 'character_mode', 'model', 'LR', 'ACC', 'f1score', 'precision', 'UAR'])
