...
class BeamSearch_RealTime(object):
    ...
    def __init__(self, trans, same, crp_alpha, num_state, beam_size, test_iteration, emo, logits, thresh):
    # Define parameters
        self.transition_bias_trans = trans
        self.transition_bias_same = same
        self.crp_alpha = crp_alpha
        self.num_state = num_state
        self.beam_size = beam_size
        self.test_iteration = test_iteration
    ...
    def decode(self, t, beam_set, utt_id, trans_pred):
        """
        Args:
            dialog: List, a list of utterances id in dialogical order.
        ...
        for i in range(num_beam):
            for j in range(self.num_state):  # => search each node
                if trans_pred >= self.thresh:
                    current_trans = self.transition_bias_trans
                    updated_beam = self._update_beam(beam_set[i].copy_beam(), utt_id, j, current_trans)
                else: 
                    current_trans = self.transition_bias_same
                    updated_beam = self._update_beam_same(beam_set[i].copy_beam(), utt_id, j, current_trans)
                
                # updated_beam = self._update_beam(beam_set[i].copy_beam(), utt_id, j, trans_pred)
                beam_set_bank.append(updated_beam)
    ...
    def _update_beam(self, beam_state, utt_id, state, crrent_trans):
        """Calculate log probability for shift and assignment process based on current beam state."""
        ...
        # Get original ce loss
        logit = np.reshape(self.all_logits[utt_id], [self.num_state,])
        loss = utils.cross_entropy(label, logit)*1
        ...
        if state in np.unique(beam_state.emo_sequence) and speaker in np.unique(beam_state.spk_sequence):
          ...
          # Find last state
            ...
            # No shift
            if state == last_state:
                loss -= np.log(1 - crrent_trans)
            # Shift
            else:
                loss -= np.log(crrent_trans) + \
                        np.log(beam_state.block_counts[state]) - \
                        np.log(sum(beam_state.block_counts) + self.crp_alpha)
                # loss -= np.log(crrent_trans) 
                beam_state.block_counts[state] += 1
    ...
    def _update_beam_same(self, beam_state, utt_id, state, crrent_trans):
        """Calculate log probability for shift and assignment process based on current beam state."""
